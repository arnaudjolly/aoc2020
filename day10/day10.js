...
Since you have some time to kill, you might as well test all of your adapters. Wouldn't want to get to your resort and realize
...
function solve_puzzle_part1(adapters) {
    adapters.sort((a, b) => a - b);

    const differences = adapters.reduce(
        (acc, adapter) => {
            const diff = adapter - acc.last;
            if (diff == 3) {
                return { last: adapter, diff1: acc.diff1, diff3: acc.diff3 + 1 };
            } else if (diff == 1) {
                return { last: adapter, diff1: acc.diff1 + 1, diff3: acc.diff3 };
            }
        }
        ,
        { last: 0, diff1: 0, diff3: 0 }
    );
    ...
    return differences.diff1 * differences.diff3;
}
...
function solve_puzzle_part2(adapters) {
    adapters.sort((a, b) => a - b);

    const cache = new Map();
    function nb_path_from(current_idx, current_elt) {
        if (cache.has(current_elt))
            return cache.get(current_elt);
        
        if (current_idx + 3 > adapters.length) {
            cache.set(current_elt, 1);
            return 1;
        }

        let sum = 0;
        if (adapters[current_idx + 3] == current_elt + 3) {
            sum += nb_path_from(current_idx + 3, adapters[current_idx + 3]);
        }
        if (adapters[current_idx + 2] == current_elt + 2) {
            sum += nb_path_from(current_idx + 2, adapters[current_idx + 2]);
        }
        sum += nb_path_from(current_idx + 1, adapters[current_idx + 1]);
        cache.set(current_elt, sum);

        return sum;
    }
    
    /* At first, we are not yet in the array */
    return nb_path_from(-1, 0);
}
