...
function solve_puzzle_part1(input, capacity) {
    const queue = [];
    for (const n of input) {
        if (queue.length >= capacity) {
            // start checking sum (reuse day01 stuff)
            let ok = find(queue, n);
            if (!ok) {
                return n;
            }
...
function solve_puzzle_part2(input, size) {
    const invalid = solve_puzzle_part1(input, size);
    for (let sum_start = size; sum_start < input.length; ++sum_start) {
        let sum = 0;
        let cursor = sum_start;
        let [min, max] = [invalid, 0];
        while(cursor < input.length && sum < invalid) {
            let n = input[cursor++];
            sum += n;
            if (n < min) min = n;
            if (n > max) max = n;
...
*/
const [input_file, size] = process.argv.slice(2);
...
ri.on('close', function(){ 
    console.log("result part1:", solve_puzzle_part1(input, size));
    console.log("result part2:", solve_puzzle_part2(input, size));
});